...
int get_numa_node_with_min_distance(int numa_node_number) {
    std::vector<int> numa_nodes_numbers = get_available_numa_nodes_numbers();
    std::vector<int> distances;
    for (size_t i = 0; i < numa_nodes_numbers.size(); i++) {
        distances.emplace_back(
            numa_distance(numa_node_number, numa_nodes_numbers[i]));
    }

    auto it = std::min_element(std::begin(distances), std::end(distances));
    int i = std::distance(std::begin(distances), it);

    return numa_nodes_numbers[i];
}
...
TEST_F(testNuma, checkModeBindMultipleNumaNodes) {
    umf_os_memory_provider_params_t os_memory_provider_params =
        UMF_OS_MEMORY_PROVIDER_PARAMS_TEST;
    ...
    int numa_node_number =
        get_numa_node_with_min_distance(numa_node_of_cpu(cpu));
